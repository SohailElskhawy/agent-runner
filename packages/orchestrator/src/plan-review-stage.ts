...
import { PLAN_REVIEW_DECISIONS } from "@agentic-dev-runner/core";
...
async function runPlanReviewStage(
  ...
  const review = parseReviewResult(agentResult.output);
 ...
 */
function parseReviewResult(output: AgentOutput): PlanReviewResult | string {
  ...
  try {
    ...
    return "malformed PLAN_REVIEW output: the agent must reply with a single JSON review object";
  }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    return "malformed PLAN_REVIEW output: the review must be a JSON object";
  }
  const record = parsed as Record<string, unknown>;
  const decision = record["decision"];
  if (typeof decision !== "string") {
    return "malformed PLAN_REVIEW output: missing or invalid review decision";
  }
  if (!isPlanReviewDecision(decision)) {
    return `malformed PLAN_REVIEW output: unknown review decision "${decision}"`;
  }
  const feedbackValue = record["feedback"];
  if (feedbackValue !== undefined && typeof feedbackValue !== "string") {
    return "malformed PLAN_REVIEW output: review feedback must be a non-empty string when provided";
  }
  if (decision === "CHANGES_REQUIRED") {
    const feedback =
      typeof feedbackValue === "string" ? feedbackValue.trim() : "";
    if (feedback.length === 0) {
      return "malformed PLAN_REVIEW output: CHANGES_REQUIRED requires actionable feedback";
    }
...
function isPlanReviewDecision(value: string): value is PlanReviewDecision {
  return (PLAN_REVIEW_DECISIONS as readonly string[]).includes(value);
}
