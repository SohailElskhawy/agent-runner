...
import type { ProcessResult, ProcessRunner, ProcessSpec } from "@agentic-dev-runner/platform";
...
export class CodexAdapter implements AgentRuntime {
  ...
  async invoke(invocation: AgentInvocation): Promise<AgentExecutionResult> {
    if (invocation.worktreePath.length === 0) {
      throw new CodexAdapterError("worktreePath must be a non-empty string");
    }
    if (!Number.isFinite(invocation.timeoutMs) || invocation.timeoutMs <= 0) {
      throw new CodexAdapterError("timeoutMs must be a positive finite number");
    }
  ...
  async probeAvailability(): Promise<AgentProbeOutcome> {
    ...
    return normalizeProbeResult(await this.runner.run(spec), CODEX_AGENT_ID);
  }
}
  ...
  switch (outcome.kind) {
    case "completed":
      ...
      return {
        ...
        failure: {
          kind: "process",
          message: `${CODEX_AGENT_ID} exited with code ${outcome.code}`,
        },
        output,
        durationMs: result.durationMs,
      };
    case "terminated":
      return {
        ...
        failure: {
          kind: "process",
          message: `${CODEX_AGENT_ID} was terminated by signal ${outcome.signal}`,
        },
        output,
        durationMs: result.durationMs,
      };
    ...
    case "spawn-error":
      return {
        ...
        failure: {
          kind: "adapter",
          message: `failed to start ${CODEX_AGENT_ID} (${outcome.code}): ${outcome.message}`,
        },
        output,
        durationMs: result.durationMs,
      };
